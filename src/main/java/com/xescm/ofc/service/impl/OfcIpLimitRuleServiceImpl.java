...
public class OfcIpLimitRuleServiceImpl extends BaseService<OfcIplimitRule> implements OfcIpLimitRuleService {
    ...
    public void checkLimit(RedisOperationUtils redisOperationUtils, HttpServletRequest request) {
        ...
        if (redisOperationUtils.hasKey("ip:" + ip + ":" + QUERY_REQUEST_COUNT)) {
            ...
            // 距第一次调用时长
            long timeDiff = currentTime - first;
            //第一阀值
            if (!isSend) {
                if (timeDiff > ipLimitRuleConfig.getFristMinTime() * 60 * 1000 && timeDiff < ipLimitRuleConfig.getFristMaxTime() * 60 * 1000) {
            ...
            //第二阀值
            if (timeDiff > ipLimitRuleConfig.getSecondMinTime() * 60 * 1000 && timeDiff < ipLimitRuleConfig.getSecondMaxTime() * 60 * 1000) {
            ...
            //第三阀值
            if (timeDiff > ipLimitRuleConfig.getThirdMinTime() * 60 * 1000) {
