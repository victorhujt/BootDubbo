...
public class  OfcPlanFedBackServiceImpl implements OfcPlanFedBackService {
    ...
    public Wrapper<List<OfcPlanFedBackResult>> planFedBackNew(OfcPlanFedBackCondition ofcPlanFedBackCondition, String userName,ConcurrentHashMap cmap) {
        ...
        try{
            ...
            if (ofcFundamentalInformation == null || ofcDistributionBasicInfo == null) {
                logger.error("传送订单号{}信息失败，查不到相关订单", orderCode);
                return null;
            }
            ...
            switch (status) {
                case "20":
                    logger.info("序号：2 ===== 订单号{}=> 跟踪状态{}", orderCode, "20-[已发运]");
                    ...
                    if (!flag) {
                        ...
                        logger.info("序号：2-insertstatus ===== 订单号{}=> 跟踪状态{}", orderCode, orderStatus.getNotes());
                    }
                    break;
                case "30":
                    logger.info("序号：3 ===== 订单号{}=> 跟踪状态{}", orderCode, "30-[已到达]");
                    flag = checkStatus(false, statusList, "start", DateUtils.Date2String(traceTime, DateUtils.DateFormatType.TYPE1)
                            + " " + "车辆已到达目的地：");
                    if (!flag) {
                        orderStatus.setLastedOperTime(traceTime);
                        ...
                        logger.info("序号：3-insertstatus ===== 订单号{}=> 跟踪状态{}", orderCode, orderStatus.getNotes());
                    }
                    break;
                case "40":
                    logger.info("序号：4 ===== 订单号{}=> 跟踪状态{}", orderCode, "40-[已签收]");
                    Date now = new Date();
                    ...
                    break;
                case "50":
                    logger.info("序号：5 ===== 订单号{}=> 跟踪状态{}", orderCode, "50-[已回单]");
                    flag = checkStatus(false, statusList, "start", DateUtils.Date2String(traceTime, DateUtils.DateFormatType.TYPE1)
                            + " " + "客户已回单");
                    if (!flag) {
                        ...
                        orderStatus.setNotes(DateUtils.Date2String(traceTime, DateUtils.DateFormatType.TYPE1)
                                + " " + "客户已回单");
                        logger.info("序号：5-insertstatus ===== 订单号{}=> 跟踪状态{}", orderCode, orderStatus.getNotes());
                    }
                    break;
                case "32":
                    logger.info("序号：6 ===== 订单号{}=> 跟踪状态{}", orderCode, "32-[中转入]");
                    ...
                    if (!flag) {
                        ...
                        orderStatus.setNotes(DateUtils.Date2String(traceTime, DateUtils.DateFormatType.TYPE1)
                                + " " + ofcPlanFedBackCondition.getDescription());
                        logger.info("序号：6-insertstatus ===== 订单号{}=> 跟踪状态{}", orderCode, orderStatus.getNotes());
                    }
                    break;
                case "34":
                    logger.info("序号：7 ===== 订单号{}=> 跟踪状态{}", orderCode, "34-[中转出]");
                    if (trimAndNullAsEmpty(ofcPlanFedBackCondition.getDescription()).equals("")) {
                        logger.info("跟踪状态已回单");
                    ...
                    if (!flag) {
                        ...
                        orderStatus.setNotes(DateUtils.Date2String(traceTime, DateUtils.DateFormatType.TYPE1)
                                + " " + ofcPlanFedBackCondition.getDescription());
                        logger.info("序号：7-insertstatus ===== 订单号{}=> 跟踪状态{}", orderCode, orderStatus.getNotes());
                    }
                    break;
                case "36":
                    ...
                    break;
                default:
                    logger.error("所给订单{}状态{}有误", orderCode, status);
                    return null;
            }
            ...
            logger.error("更新订单{}状态发生未知异常：异常详情 => {}", orderCode, e);
            return null;
        }
        return null;
    }
    ...
    public Wrapper<List<OfcPlanFedBackResult>> schedulingSingleFeedbackNew(OfcSchedulingSingleFeedbackCondition ofcSchedulingSingleFeedbackCondition, String userName) {
        for (int i = 0; i < ofcSchedulingSingleFeedbackCondition.getOrderCode().size(); i++) {
            //注意，运输单号即是订单号
            String orderCode = trimAndNullAsEmpty(ofcSchedulingSingleFeedbackCondition.getOrderCode().get(i));
            try {
                if (orderCode.equals("") || !trimAndNullAsEmpty(orderCode).startsWith(ORDER_PRE)) {
                    logger.error("订单号为空或者格式不正确: [{}]", orderCode);
                    return null;
                }
                if (ofcSchedulingSingleFeedbackCondition.getDeliveryNo().equals("")) {
                    logger.error("调度单号不可以为空");
                    return null;
                }
                if (ofcSchedulingSingleFeedbackCondition.getCreateTime() == null) {
                    logger.error("调度单时间不可以为空");
                    return null;
                }
                logger.info("##################transPortNo为：{}", orderCode);
                //更新车牌号、司机姓名、联系电话
                OfcDistributionBasicInfo ofcDistributionBasicInfo = ofcDistributionBasicInfoService.distributionBasicInfoSelect(orderCode);
                ...
                logger.error("更新订单{}调度信息发生异常：异常详情 => {}", orderCode, ex);
            } catch (Exception ex) {
                logger.error("更新订单{}调度信息发生未知异常：异常详情 => {}", orderCode, ex);
            }
        }
        return null;
    }
