    ...
    $(function(){
        ...
        $("#goodsEnter").click(function () {
            ...
            $("#goodsInfoListDiv").find("tr").each(function(index){
                ...
                var quantity = tdArr.eq(5).children().val();//    数量
                var production_batch = tdArr.eq(6).children().val();//    批次
                var production_time = tdArr.eq(7).children().val();//    生产日期
                var invalid_time = tdArr.eq(8).children().val();//    失效日期
                debugger;
